...
describe('ERC721', function () {
    this.timeout(0);
    const [admin, owner, newOwner, other, publisher, platform, nftContract, approved, anotherApproved, operator] = accounts;
    ...
    beforeEach(async function () {
        this.accessControls = await HashtagAccessControls.new({from: admin});
        this.token = await HashtagProtocol.new(this.accessControls.address, admin, {from: admin});

        // add a publisher to the protocol
        await this.accessControls.grantRole(web3.utils.sha3('PUBLISHER'), publisher, {from: admin});
    });
    ...
    describe('metadata', function () {
        ...
        describe('token URI', function () {
            ...
            it('can be set for a token id', async function () {
                await this.token.setTokenURI(firstTokenId, sampleUri, {from: admin});
                expect(await this.token.tokenURI(firstTokenId)).to.be.equal(sampleUri);
            });

            it('reverts when setting for non existent token id', async function () {
                await expectRevert(
                    this.token.setTokenURI(nonExistentTokenId, sampleUri, {from: admin}), 'ERC721Metadata: URI set of nonexistent token'
                );
            });
        ...
        describe('tokenByIndex', function () {
            ...
            [firstTokenId, secondTokenId].forEach(function (tokenId) {
                it(`should return all tokens after burning token ${tokenId} and minting new tokens`, async function () {
                    ...
                    await this.token.mint(thirdHashtag, publisher, newOwner, {from: publisher, value: ether('0.1')});
                    await this.token.mint(forthHashtag, publisher, newOwner, {from: publisher, value: ether('0.1')});
    ...
    describe('_mint(address, uint256)', function () {
        it('reverts with a null destination address', async function () {
            await expectRevert(
                this.token.mint(thirdHashtag, publisher, ZERO_ADDRESS, {from: publisher, value: ether('0.1')}),
        ...
        context('with minted token', async function () {
            beforeEach(async function () {
                ({logs: this.logs} = await this.token.mint(firstHashtag, publisher, owner, {
                    from: publisher,
                    value: ether('0.1')
                }));
            });
    ...
    describe('_burn', function () {
        ...
        //FIXME need to check we can get the contract in this state
        it.skip('reverts when burning a non-existent token id', async function () {
            await expectRevert(
                this.token.burn(firstTokenId, {from: owner}), 'ERC721: owner query for nonexistent token'
            );
        });
        ...
        context('with minted tokens', function () {
            ...
            context('with burnt token', function () {
                beforeEach(async function () {
                    ({logs: this.logs} = await this.token.burn(firstTokenId, {from: owner}));
                });
